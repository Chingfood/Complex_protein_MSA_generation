...
## This script calls download_xref.py
## Multi process for using ENA cross reference service to convert UniprotID to EMBL ID. 
...
do
	...
	nohup python3 $MY_PATH/util/download_xref.py $ID_list $source $outfile $l $r > $logfile 2>$errfile  &
	sleep 3
done
wait
