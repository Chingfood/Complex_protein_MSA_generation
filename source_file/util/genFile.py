...
content_dtype = {'names': ('uniprot', 'uniref', 'contig', 'start', 'end', 'length'), 'formats': ('S10', 'S10', 'S20', 'i4', 'i4', 'i4')}
def func(argss):
...
content.sort(order=['uniref', 'contig', 'rank'])

with open(outfile, 'w') as f:
	f.write('\n'.join(['\t'.join([c['uniref'].decode(), c['contig'].decode(), str(c['rank'])]) for c in content]) + '\n')
