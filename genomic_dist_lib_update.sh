...
cat $WORK_DIR/uniref_id_UNI_UNI $WORK_DIR/uniref_id_UPI_UNI_multiprocess > $uniprot_ID_list
...
echo "We have $num_query queries in total"
from=0; to=$num_query;  num_thread=50   # left-close, right-open
bash $MY_PATH/source_file/download_contig.sh $ID_list $outfolder $from $to $num_thread
wait
...
awk '{printf("%s\t%s\t%s\n",$2,$6,$4)}' $merged_xref_list > $WORK_DIR/tmp_merged_xref_list
...
python3 $MY_PATH/source_file/util/genFile.py $loc_folder $WORK_DIR/id_loc_${version}_redundant
python3 $MY_PATH/source_file/util/file_line_redundancy_remove.py $WORK_DIR/id_loc_${version}_redundant $id_loc

## This will have the UnirefID started with UniRef100_
awk '{printf ("UniRef100_%s\t%s\t%s\n",$1,$2,$3) }' $id_loc > $WORK_DIR/id_loc_${version}_uniref
...
#rm $WORK_DIR/uniref_id_UNI_UNI
#rm $WORK_DIR/uniref_id_UPI_UNI_multiprocess 
#
#rm -f $WORK_DIR/tmp_contig $WORK_DIR/tmp_coding
#
#rm $WORK_DIR/tmp_merged_xref_list
#
#rm $WORK_DIR/id_loc_${version}_redundant
