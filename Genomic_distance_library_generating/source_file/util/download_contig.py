...
## This script use enaBrowserTools enaDataGet.py to download contig file.
...
#enaDataGet = "/home/qingyliu/ENA/enaBrowserTools/python3/enaDataGet.py"
enaDataGet = os.path.join(current_file_dir, "enaBrowserTools/python3/enaDataGet.py")
enaDataGet_exe = os.path.join(current_file_dir, "enaBrowserTools/python3/enaDataGet")
...
def execute_download(path,executable_path,ID,wgs = False):
   ## This function to use enaDataGet to download contig file.
   ## This function carrys out the specific downloading step.
...
def download_embl(path,executable_path,ID,wgs = False, counter_limit = 2):
    ## This function check whether downloading works and do some after processing.

    counter = 0
    flag = -10
    while (flag != 0 and counter < counter_limit):
...
for i, ID in enumerate(query_list):
    ...
    print('%d\t%s\tfinished' % (lindex+i, ID))
    sys.stdout.flush()
    i += 1
